...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Sync Clerk user with our user state and backend
  useEffect(() => {
    if (isLoaded && isSignedIn && clerkUser) {
      ...
      console.log('[AUTH] Syncing user to backend:', mappedUser.id);
      fetch('http://192.168.1.6:8000/api/syncUser', {
        method: 'POST',
        headers: {
        ...
        .then(res => res.json())
        .then(data => {
          console.log('[AUTH] User synced to backend:', data);
        })
        .catch(error => {
